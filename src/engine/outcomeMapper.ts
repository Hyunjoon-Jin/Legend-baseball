...
/** Runners scoring on a hit, very roughly modeled. */
function runsOnHit(basesReached: number, situation: GameSituation, rng: () => number): number {
  const { runners } = situation;
  let runs = 0;
  if (basesReached >= 4) return countRunners(situation); // home run: everyone scores

  if (runners.third) runs += 1; // runner on third virtually always scores on a hit
  if (runners.second) {
    if (basesReached >= 2) runs += 1; // double+ always scores runner from second
    else if (rng() < 0.45) runs += 1; // single: ~45% scores from second
  }
  if (runners.first && basesReached >= 3) runs += 1; // triple scores runner from first
...
function resolveGroundBall(
  ...
  if (rng() < hitProb) {
    ...
    return { result, basesReached: 1, runsScoredEstimate: runsOnHit(1, situation, rng) };
  }
  ...
  if (runners.first && outs < 2) {
    ...
    if (runners.second && outs === 0 && rng() < dpProb * 0.05) {
      return { result: 'triplePlay', basesReached: 0, runsScoredEstimate: 0 };
    }
    if (rng() < dpProb) {
      return { result: 'doublePlay', basesReached: 0, runsScoredEstimate: 0 };
    }
  }

  // Fielder's choice: a runner is forced out elsewhere, batter reaches.
  if (countRunners(situation) > 0 && outs < 2 && rng() < 0.08) {
    return { result: 'fieldersChoice', basesReached: 1, runsScoredEstimate: 0 };
  }

  // Hard-hit grounders occasionally squeak through on a fielding error.
  if (profile.exitVelocity > 100 && rng() < 0.03) {
    return { result: 'reachedOnError', basesReached: 1, runsScoredEstimate: runsOnHit(1, situation, rng) };
  }

  return { result: 'groundOut', basesReached: 0, runsScoredEstimate: 0 };
}
...
function resolveLineDrive(
  ...
  if (rng() < hitProb) {
    ...
    if (profile.distance >= fenceDistance && profile.exitVelocity >= 140) {
      return { result: 'homeRun', basesReached: 4, runsScoredEstimate: runsOnHit(4, situation, rng) };
    }
    if (profile.distance >= fenceDistance * 0.85) {
      const triple = isGap && batter.speed > 65 && rng() < 0.35;
      const basesReached = triple ? 3 : 2;
      return {
        result: triple ? 'triple' : 'double',
        basesReached,
        runsScoredEstimate: runsOnHit(basesReached, situation, rng),
      };
    }

    return { result: 'single', basesReached: 1, runsScoredEstimate: runsOnHit(1, situation, rng) };
  }
  ...
  return { result: 'lineOut', basesReached: 0, runsScoredEstimate: 0 };
}
...
function resolveFlyBall(
  ...
  if (ratio >= 0.95 && isGap && batter.speed > 80 && rng() < 0.15) {
    return { result: 'insideTheParkHomeRun', basesReached: 4, runsScoredEstimate: runsOnHit(4, situation, rng) };
  }
  ...
  if (situation.runners.third && situation.outs < 2 && ratio >= 0.55) {
    return { result: 'sacrificeFly', basesReached: 0, runsScoredEstimate: 1 };
  }

  return { result: 'flyOut', basesReached: 0, runsScoredEstimate: 0 };
}
...
function resolvePopUp(profile: BattedBallProfile, situation: GameSituation, rng: () => number): BattedBallOutcome {
  ...
  if (rng() < errorProb) {
    return { result: 'reachedOnError', basesReached: 1, runsScoredEstimate: 0 };
  }
  return { result: 'popOut', basesReached: 0, runsScoredEstimate: 0 };
}

/**
 * Resolves a batted-ball profile into a fully detailed plate appearance
 * result (hit type, out type, double play, sacrifice fly, etc.), along
 * with the bases reached and a rough estimate of runs scored.
 */
